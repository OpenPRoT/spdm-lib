...
impl SpdmCertStore for DemoCertStore {
    ...
    fn root_cert_hash<'a>(
        ...
        #[cfg(feature = "crypto")]
        {
            // Extract only the root certificate (first certificate in the chain)
            // since our chain is now: Root CA (first) + Attestation (second)
            let root_cert_der = self.extract_first_certificate_der(&self.cert_chain)
                .ok_or(CertStoreError::CertReadError)?;
            
            // Hash only the root certificate using SHA-384
            let mut hasher = Sha384::new();
            hasher.update(root_cert_der);
            let result = hasher.finalize();
...
fn display_info(config: &ResponderConfig) {
    ...
    println!("  Certificates: Static OpenSSL-generated certificate chain");
    println!();
}
